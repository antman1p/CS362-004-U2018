 ...
 */
 ...
 void testCard(int player, struct gameState *post)
 {
	...
	// Check Results
	// Requirement:  Player  receives exactly 1 card
	printf("\nPlayer 1 receives one card\n");
	printf("Expected: %d\n", pre.handCount[player]);
	printf("Result: %d\n", post->handCount[player]);
	assertTrue(post->handCount[player], pre.handCount[player]);
	// Village card function also discards a card, so hand counts should be equal
	
	
	// Requirement:  Player  receives exactly 1 card from their own pile
	...
	printf("Expected: %d\n", pre.numActions + 2);
	printf("Result: %d\n", post->numActions);
	assertTrue(post->numActions, pre.numActions + 2);
